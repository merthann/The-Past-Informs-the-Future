...
# League mapping for directory names (sadece 4 lig - Premier League hariç)
LEAGUE_MAP = {
...
print(f"Leagues: {list(LEAGUE_MAP.keys())}")
print("=" * 60)
...
def create_scraper():
    ...
    # Sadece headless parametresi kalsın, hata veren selenium_options silindi
...
for idx, match_info in enumerate(all_matches, 1):
    ...
    # Desteklenen lig mi kontrol et
    if league not in LEAGUE_MAP:
        print(f"\n[{idx}/{len(all_matches)}] Match {match_id}: ⏭️  Skipped (unsupported league {league})")
        continue
    ...
    # Klasör yolu oluşturma fonksiyonu (hepsi minute{X}_otherleagues/{league}/ yapısında)
    def get_time_output_dir(time_val):
        return base_data_dir / f"minute{time_val}_otherleagues" / league_dir_name
    
    # Önce 45, 60, 75, 90'ın hepsinin olup olmadığını kontrol et
    existing_periods_all_present = True
    for time_cutoff in EXISTING_PERIODS_TO_CHECK:
        time_output_dir = get_time_output_dir(time_cutoff)
        match_dir = time_output_dir / f"match_{match_id}"
        if not has_all_required_files(match_dir):
            existing_periods_all_present = False
            break
    
    # Eğer 45, 60, 75, 90 varsa, sadece 30'u kontrol et
    if existing_periods_all_present:
        time_30_dir = get_time_output_dir(30)
    ...
    for time_cutoff in periods_to_process:
        # Her zaman dilimi için ayrı klasör (get_time_output_dir fonksiyonu kullanılıyor)
        time_output_dir = get_time_output_dir(time_cutoff)
