...
def _get_active_players_at_time(
    events: pd.DataFrame,
    ...
    """Get players currently on field at given minute.
    
    Starts with starting 11, then applies substitutions and red cards up to at_minute.
    
    Args:
        events: All match events
        players: All players (starters + subs)
        team_id: Team ID to filter events
        team_side: 'home' or 'away'
        at_minute: Get roster as of this minute
    
    Returns:
        DataFrame of players currently on field (may be < 11 if red cards)
    """
    time_col = _get_time_col(events)
    
    # Start with starting 11
    active_ids = set(
        players[(players['team_side'] == team_side) & (players['is_first_eleven'] == True)]
        ['player_id'].astype(int).tolist()
    )
    
    # Get substitutions before at_minute for this team
    if not events.empty and 'type' in events.columns:
        team_events = events[
            (events['teamId'] == team_id) & 
            (events[time_col] < at_minute)
        ]
        
        # Process SubstitutionOff - remove player
        sub_offs = team_events[team_events['type'] == 'SubstitutionOff']
        for _, row in sub_offs.iterrows():
            pid = row.get('playerId')
            if pd.notna(pid):
                active_ids.discard(int(pid))
        
        # Process SubstitutionOn - add player
        sub_ons = team_events[team_events['type'] == 'SubstitutionOn']
        for _, row in sub_ons.iterrows():
            pid = row.get('playerId')
            if pd.notna(pid):
                active_ids.add(int(pid))
        
        # Process Red Cards - remove player
        cards = team_events[team_events['type'] == 'Card']
        for _, row in cards.iterrows():
            quals = str(row.get('qualifiers', ''))
            # Check for red card or second yellow
            if 'Red' in quals or 'SecondYellow' in quals:
                pid = row.get('playerId')
                if pd.notna(pid):
                    active_ids.discard(int(pid))
    
    # Return DataFrame of active players
    return players[players['player_id'].isin(active_ids)].copy()
...
def create_interval_graph(
    ...
    # Get players on field at start of interval (handles subs and red cards)
    team_players = _get_active_players_at_time(events, players, team_id, team_side, start_min)
